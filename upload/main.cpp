...
#if defined(NDEBUG)
# error "Milkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3d7a0ded35c5cabb4243251619f2a99e030f35789348ce2dda87f626946466da");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Milkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Milkcoin Signed Message:\n";
...
{
    // Milkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 52500 blocks, which will occur approximately in ~1/4 year
    nSubsidy >>= (nHeight / 52500); // Milkcoin: 52k blocks in ~1/4 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Milkcoin: 1 days
static const int64 nTargetSpacing = 4 * 60; // Milkcoin: 4 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Milkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Milkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1317907ba0d416dc683e88a5111a92058038be6e1de600855908764d18bdb448");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "05 October 2020";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048fc2ee8edda876464f31e425242c2764674e3dd5850aaffbbeba046d57efdd686d2877a29e4cafb024bc4117a66c3b71087e5481518b08ff42ddc4198e655dfd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1601914622;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085094673;

        if (fTestNet)
        {
            block.nTime    = 1601914601;
            block.nNonce   = 386192984;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xce457bd562315864f7678ded05ff15068226df0f2565acec503771bcd09c76b6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xcl, 0xb3, 0xde }; // Milkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MilkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MilkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MilkcoinMiner(CWallet *pwallet)
{
    printf("MilkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("milkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MilkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MilkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MilkcoinMiner, pwallet));
}
